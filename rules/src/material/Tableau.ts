...
import { AtLeastOneShield, Condition, ConditionType } from './Condition'
import { EffectType } from './Effect'
import { keys } from './Key'
import { LocationType } from './LocationType'
import { MaterialType } from './MaterialType'
import { Place } from './Place'
import { hasLock, hasPurse } from './Scoring'
...
export class Tableau extends MaterialRulesPart {
  ...
  countCondition(condition: Condition, x?: number, y?: number): number {
    switch (condition.type) {
      ...
      case ConditionType.PerMissingShieldType:
        return shields.filter((shield) => !this.cards.some((card) => card && cardCharacteristics[card].shields.some((cardShield) => cardShield === shield)))
          .length
      ...
      case ConditionType.PerShieldsSet:
        return this.countSets(
          condition.shields,
          this.cards.flatMap((card) => (card ? cardCharacteristics[card].shields : []))
        )
      ...
      case ConditionType.SumOfCost:
        return this.getSumOfCost(x!)
      case ConditionType.PerDifferentCost:
        return this.getDifferentCost()
      case ConditionType.IfDiscountMissing:
        return this.hasDiscountMissing() ? this.getCardScore(x!, y!) : 0
      case ConditionType.AtLeastOneShield:
        return this.hasOneShield(condition, x!, y!) ? this.getCardScore(x!, y!) : 0
      case ConditionType.IfPlaceMissing:
        return this.haventPlace() ? this.getCardScore(x!, y!) : 0
      case ConditionType.IfCardWithPurseMissing:
        return this.haventPurse() ? this.getCardScore(x!, y!) : 0
      case ConditionType.PerLock:
        return this.countCards(hasLock)
      default:
        return 0
    }
  }
  ...
  getCardIndex(x: number, y: number) {
    return this.material(MaterialType.Card)
      ...
      .player(this.player)
      .location((l) => l.x === x! + this.xMin && l.y === y! + this.yMin)
      .getIndex()
  }

  getDiscount(place: Place) {
    const tableau = this.material(MaterialType.Card).location(LocationType.Tableau).player(this.player).getItems<CardId>()
  ...
  getDifferentCost() {
    ...
    const allCost = cards.map((card) => cardCharacteristics[card].cost)
    return new Set(allCost).size
  }

  private getSumOfCost(x: number) {
    const tableau = this.material(MaterialType.Card).location(LocationType.Tableau).player(this.player).getItems<CardId>()
    const row = tableau.filter((t) => t.location.x === x && t.location.rotation)
    const costCards = row.map((card) => cardCharacteristics[card.id.front!].cost)
    return costCards.reduce((acc, cur) => acc + cur, 0)
  }

  private hasDiscountMissing() {
    return this.cards.some((card) => cardCharacteristics[card!].effects.some((effect) => effect.type !== EffectType.Discount))
  }

  private hasOneShield(condition: AtLeastOneShield, x: number, y: number) {
    const tableau = this.material(MaterialType.Card).location(LocationType.Tableau).player(this.player).getItems<CardId>()
    const group = tableau.filter((card) => (condition.column ? card.location.x === x : card.location.y === y))
    return group.some((card) => cardCharacteristics[card.id.front!].shields.some((shield) => condition.shield === shield))
  }

  private haventPlace() {
    const tableau = this.material(MaterialType.Card).location(LocationType.Tableau).player(this.player).getItems<CardId>()
    return tableau.every((card) => !card.location.rotation)
  }

  private haventPurse() {
    return this.cards.every((card) => cardCharacteristics[card!].scoring.condition.type !== ConditionType.PerGoldInPurse)
  }
}
